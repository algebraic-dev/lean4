...
public import Std.Internal.Http.Protocol.H1.Machine.Event
public import Std.Internal.Http.Protocol.H1.Machine.State
public import Std.Internal.Http.Protocol.H1.Machine.Config
...
@[inline]
def setRequest (request : Data.Request.Head) (reader : Reader) : Reader :=
  { reader with request }
...
--/
private def determineBodySize (writer : Writer) : Data.Body.Length :=
...
-/
private def writeHeaders (response : Data.Response.Head) (writer : Writer) : Writer :=
...
@[inline]
def setFailure (machine : Machine) (error : H1.Machine.Error) (status : Data.Status) : Machine :=
  machine
  |>.addEvent .failed
...
private def parseWith (machine : Machine) (parser : Parser α) (limit : Option Nat) (expect : Option Nat := none) : (Machine × Option α) :=
  ...
  | .error it .eof =>
    -- Used to avoid creating a huge request through a huge request.
    let usedBytesUntilFailure := remaining - it.remainingBytes

    if let some limit := limit then
      if usedBytesUntilFailure ≥ limit then
        (machine.setError .badRequest, none)
      else
...
def getRequestSize (req : Data.Request.Head) : Option Data.Body.Length := do
...
-/
def setHeaders (response : Data.Response.Head) (machine : Machine) : Machine :=
...
def processRead (machine : Machine) : Machine :=
  match machine.reader.state with
  | .needStartLine =>
    let (machine, result) := parseWith machine Parser.parseRequestLine (limit := some 8192)
  ...
  | .needHeader headerCount =>
    let (machine, result) := parseWith machine (Parser.parseHeaderLine machine.config.maxHeaderSize) (limit := none)
  ...
  | .needChunkedSize =>
    let (machine, result) := parseWith machine Parser.parseChunkSize (limit := some 128)

    match result with
    | some (size, ext) =>
      machine
      |>.setReaderState (.needChunkedBody size)
      |>.setEvent (ext <&> (.chunkExt ∘ ByteSlice.toByteArray))
    | none =>
      machine

  | .needChunkedBody size =>
    let (machine, result) := parseWith machine (Parser.parseChunkedSizedData size) (limit := none) (some size)
